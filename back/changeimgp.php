...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    $extension = explode(".", $_FILES['updfot']['name']);
    if($_POST['nimg']!=""){
        $nfile = explode(".", $_POST['nimg']);
        $file = $nfile[0].".".$extension[1];
    }
    else{
    ...
    if ($file && move_uploaded_file($_FILES['updfot']['tmp_name'],"../public/img/prods/".$file)) {
