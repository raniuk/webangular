...
class PDF extends FPDF {
    ...
    function Header(){
        ...
        $this->Cell(0,20, 'REPORTE DE CATEGORIAS', 0, 1, 'C');
        ...
        $this->Cell(160,6,'DESCRIPCION',1,0,'C',true);
...
if (count($vdes)>0) {
    $pdf->SetWidths(array(9,160,17));
    $pdf->SetAligns(array('L','L','C'));
    for($i=0;$i<count($vdes);$i++){        
        $pdf->Row(array($i+1,utf8_decode($vdes[$i]),$vest[$i]));
    }
    $pdf->Output();
}
else {
