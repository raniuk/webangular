<?php session_start();
    ...
    getProperties()
        ...
        ->setTitle("Reporte diaria de servicios")
        ->setSubject("Reporte diaria")
        ->setDescription("Reporte diaria de servicios")
        ->setKeywords("GMZBol")
        ->setCategory("Reportes");
    $objPHPExcel->setActiveSheetIndex(0)->mergeCells('A1:G1');
    //$objSheet = $objPHPExcel->setActiveSheetIndex(0);
    $objSheet = $objPHPExcel->setActiveSheetIndex();
    $objSheet->setCellValue('A1', 'REPORTE DIARIA DE SERVICIOS'." - ".substr($fecha,8,10)." de ".$mes." de ".substr($fecha,0,4));
    ...
    $resultado=$conn->query("SELECT ps.bancoempse, ps.formapuse, ps.formapdse, ps.formaptse, ps.formapcse, ps.tiempopse, ps.numeropse, cl.nombrecli, cl.razonempcli FROM clientes cl, prestarserv ps WHERE ps.idu='$user' AND ps.fechapse='$fecha' AND cl.idcli=ps.idcli ORDER BY ps.idpse");
    $i = 3;
    ...
    header('Content-Disposition: attachment;filename="rsdiaria.xls"');
