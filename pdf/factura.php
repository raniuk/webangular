...
class PDF extends FPDF {
    ...
    function Header(){
        ...
        $selectuven=$conn->query("SELECT numeroven FROM ventas WHERE idu='$user' AND idcli='$cliente' AND fechaven='$fecha' ORDER BY idven DESC LIMIT 1")->fetch();
...
$user = $_SESSION['iop'];
$fecha = date("Y-m-d");
$selectuven=$conn->query("SELECT idven, bancoempven, formapuven, detallefpuven, formapdven, detallefpdven, formaptven, detallefptven, formapcven, detallefpcven, tiempoven, descuentoven, validezofertaven FROM ventas WHERE idu='$user' AND idcli='$cliente' AND fechaven='$fecha' ORDER BY idven DESC LIMIT 1")->fetch();
